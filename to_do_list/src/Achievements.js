...
function Achievements() {
    ...
    return (
        <div className="bg-gradient-to-b  min-h-screen p-4 text-white">


            {/* Header with XP and Level */}
            <div className="relative p-4 text-white flex flex-col gap-4  bg-gradient-to-b from-gray-900 to-purple-900 rounded-xl mb-6">
                <div className="flex flex-row items-center gap-4 justify-start">
                    ...
                    <div className="flex flex-col gap-1">
                        ...
                        <p className="italic text-md">"Saving the day, one task at a time!"</p>
                    </div>
                </div>
                ...
                <div className="mb-2">
                    ...
                    <div className="w-full bg-white/30 rounded-full h-2.5">
                        <div
                            className="bg-purple-300 h-2.5 rounded-full transition-all duration-500"
                            style={{ width: `${(currentXP % 1000 / 1000) * 100}%` }}
                        ></div>
                    </div>
                </div>

                <div className="absolute right-4 top-4 bg-purple-800 text-white font-bold text-sm px-4 py-2 rounded-xl shadow-lg flex items-center justify-center space-x-2 hover:scale-105 transition cursor-pointer">
            ...
            <div className="flex border-b border-purple-700 mb-6">
                <button
                    className={`px-4 py-2 font-medium ${activeTab === 'achievements' ? 'text-purple-300 border-b-2 border-purple-300' : 'text-purple-100'}`}
                    onClick={() => setActiveTab('achievements')}
                >
                    Achievements
                </button>
                <button
                    className={`px-4 py-2 font-medium ${activeTab === 'avatars' ? 'text-purple-300 border-b-2 border-purple-300' : 'text-purple-100'}`}
                    onClick={() => setActiveTab('avatars')}
                >
                    Avatars
                </button>
                <button
                    className={`px-4 py-2 font-medium ${activeTab === 'stats' ? 'text-purple-300 border-b-2 border-purple-300' : 'text-purple-100'}`}
            ...
            <div className="mb-8">
                {activeTab === 'achievements' && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {achievements.map(achievement => (
                            <div
                                key={achievement.id}
                                className={`p-4 rounded-lg border ${achievement.unlocked ? 'bg-purple-800/50 border-purple-500' : 'bg-gray-800/50 border-gray-700'}`}
                            >
                                <div className="flex items-center gap-3">
                                    <div className={`w-10 h-10 rounded-full flex items-center justify-center ${achievement.unlocked ? 'bg-yellow-500' : 'bg-gray-700'}`}>
                ...
                {activeTab === 'avatars' && (
                    <div className="grid grid-cols-4 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-7 gap-4">
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                                {badges.map(badge => (
                                    <div key={badge.id} className="text-center">
                                        <div className={`w-16 h-16 mx-auto rounded-full border-2 ${badge.unlocked ? 'border-yellow-400' : 'border-gray-600 opacity-50'}`}>
                                            <img
                                        ...
                                        {!badge.unlocked && <p className="text-xs text-gray-400">Locked</p>}
                                    </div>
                                ))}
                            </div>
                        </div>
...
function StatCard({ title, value, icon }) {
    return (
        <div className="bg-purple-800/50 p-3 rounded-lg flex items-center gap-3">
...
function ChallengeTask({ title, progress, total, xpReward, onComplete }) {
    ...
    return (
        <div
            className={`flex items-center gap-3 p-3 rounded-lg ${checked ? 'bg-green-900/30' : 'bg-purple-900/30'} cursor-pointer hover:bg-purple-800/50 transition`}
            onClick={handleCheck}
        >
            <div className={`w-6 h-6 rounded-full flex items-center justify-center ${checked ? 'bg-green-500' : 'border-2 border-white'}`}>
                {checked ? '✓' : ''}
            </div>
